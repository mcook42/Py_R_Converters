...
indents  =  :
# Tested-Works
def createRfile():    """Creates a new file named "filename.R" by removing .py extension"""
...
def find_all(tofind, string):    """Returns number of times a certain substring is found"""
    found  =  [i for i in range(len(string)) if ((string.startswith(tofind, i)]
    num_found  =  len(found)
    return num_found

def complexchange(line):    """Completes multi-step line changes """    
...
# TESTED-Works   
def chglinecontent(line):    """Changes content contained within a single line"""
...
def indentation(s, tabsize=4):    """Generator to return level of indentation"""
...
def stringify(sub):    """Returns python string versions ('' and "") of original substring"""
    check_str_s  =  "'" + sub + "'"
    check_str_d  =  '"' + sub + '"'
    return check_str_s, check_str_d

def setflags(line, indents):    """Scans line to set/unset flags for further processing"""
...
def standind(line, cur_ind):    """Standardizes indentation"""
...
#TESTED-WORKS
def statement(line):    """Converts if ((statements"""
...
def ignoreStrReplace(line, cur, rep):    """Wrapper for str.replace to ignore strings"""
    if ('"' in line)){
...
def closeBrackets(file):    """Attempts to find and close the opened brackets"""
...
def main():    pyfile, rfile  =  createRfile()
    ...
    if (flags['bracket']):
def print("This ality may not work perfectly... ")        toClose = input("""
